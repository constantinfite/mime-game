<template>
  <v-container class="full-height flex-column d-flex">
    ...
    <v-row v-if="!finish" justify="center">
      <v-btn small color="#E71D36" class="white--text mr-5" @click="skipWord"> Passe </v-btn>
      <v-btn small color="#2E294E" class="white--text mr-5" @click="nextWord"> Suivant </v-btn>
      <v-btn v-if="!showWord" small color="#2E294E" class="white--text" @click="showWordFunction">
        Montrer le mot
      </v-btn>
    </v-row>
    <v-row justify="center" align="center" class="mt-1 flex-column d-flex">
      ...
      <div v-if="!finish">
        <!--     Start Timer -->
        <v-icon v-if="!timer" x-large class @click="startTimer"> mdi-play-circle-outline </v-icon>
        <!--     Pause Timer -->
        <v-icon v-if="timer" x-large class="mr-5" @click="stopTimer"> mdi-pause-circle </v-icon>
        <!--     Restart Timer -->
        <v-icon v-if="resetButton" x-large @click="resetTimer"> mdi-replay </v-icon>
      </div>
    </v-row>
    <v-row justify="center" class="div-bottom">
      <v-btn v-if="finish" small color="#f46036" class="mb-5 white--text" @click="switchTeam"> Switch Team </v-btn>
    </v-row>
  </v-container>
</template>

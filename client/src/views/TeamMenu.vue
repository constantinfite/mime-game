  ...
  <v-container>
      ...
      <v-col cols="8">
        ...
        <v-sheet elevation="0" class="pa-4">
          <v-chip-group column active-class="primary--text">
            <v-chip v-for="player in redTeam" :key="player.id">{{ player }}</v-chip>
          </v-chip-group>
        </v-sheet>
        <h3>Team Bleu</h3>
        <v-sheet elevation="0" class="pa-4">
          <v-chip-group column active-class="primary--text">
            <v-chip v-for="player in blueTeam" :key="player.id">{{ player }}</v-chip>
          </v-chip-group>
        </v-sheet>
      </v-col>
    </v-row>
    ...
    <v-row justify="center">
      <v-btn color="#1B998B" small class="link">
        <router-link to="/Game">Lancer la partie</router-link>
      </v-btn>
    </v-row>
